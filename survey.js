const params=new URLSearchParams(location.search),eventId=params.get("event"),accessToken=params.get("token");
...
async function loadSurvey(){if(!accessToken){showError("توکن پرسشنامه در لینک وجود ندارد.");return}if(!window.CloudSurveyApi||typeof window.CloudSurveyApi.load!=="function"){showError("فایل cloud-survey.js بارگذاری نشده است.");return}try{const loaded=await window.CloudSurveyApi.load(accessToken,eventId);loadedSurvey=loaded.event}catch(error){console.error(error);showError("پرسشنامه هنوز روی این گوشی ذخیره نشده و ارتباط اینترنتی برقرار نیست.");return}if(!loadedSurvey){showError("این لینک معتبر نیست یا پرسشنامه منقضی شده است.");return}document.getElementById("event-time").textContent=formatDateTime(loadedSurvey.scheduled_for||loadedSurvey.window_opened_at||loadedSurvey.created_at);document.getElementById("occupant-code").textContent=`کد ناشناس شما: ${loadedSurvey.occupant_code||"---"}`;if(loadedSurvey.status==="COMPLETED"){showView("success");return}if(["EXPIRED","UNANSWERED","CLOSED"].includes(loadedSurvey.status)){showView("closed");return}showView("intro")}
function getTehranTimestamp(){const parts=new Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Tehran",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hourCycle:"h23"}).formatToParts(new Date()),v=Object.fromEntries(parts.filter(p=>p.type!=="literal").map(p=>[p.type,p.value]));return`${v.year}-${v.month}-${v.day}T${v.hour}:${v.minute}:${v.second}+03:30`}
function numericValue(data,key){const value=data.get(key);return value===null||value===""?null:Number(value)}
